...
- **URL:** https://core.telegram.org/bots/api/available-types/chatmemberadministrator
- **Summary:** ChatMemberAdministrator that has some additional privileges. | status | String | The member's status in the chat, always “administrator” | | can_be_edited | Boolean | _True_, if the bot is allowed to edit administrator privileges of that user | | is_anonymous | Boolean | _True_, if the user's...
