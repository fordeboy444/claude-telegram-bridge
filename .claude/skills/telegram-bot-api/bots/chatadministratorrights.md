...
- **URL:** https://core.telegram.org/bots/api/available-types/chatadministratorrights
- **Summary:** ChatAdministratorRights Represents the rights of an administrator in a chat. | is_anonymous | Boolean | _True_, if the user's presence in the chat is hidden | | can_manage_chat | Boolean | _True_, if the administrator can access the chat event log, get boost list, see hidden supergroup and channel...
...
| can_manage_tags | Boolean | _Optional_. _True_, if the administrator can edit the tags of regular members; for groups and supergroups only |
| can_send_welcome_messages | Boolean | _True_, if the administrator can manage chat welcome messages or directly send them in the case of bots |
