# Agents & sessions — Orca Docs

- **URL:** https://www.onorca.dev/docs/model/agents-sessions
- **Summary:** State dots, restart chips, and the lifecycle of an agent session.
...
![Inline agent status on every worktree card — yellow while working, green when done](https://www.onorca.dev/whats-new/posters/agent-statuses.jpg)
...
You can assign a keyboard shortcut to **Toggle Agent Dashboard** under [Settings → Shortcuts](https://www.onorca.dev/docs/settings#shortcuts)
. It has no default binding, and works while a terminal, editor, or browser tab has focus.
...
Dashboard cards show a host badge for SSH workspaces and paired [Remote Orca Servers](https://www.onorca.dev/docs/remote-servers)
. Hover or focus the badge to see the saved host name, such as **SSH host · openclaw** or **Remote Orca host · Build Mac**. Local workspaces do not show a host badge.
...
When an agent exits (clean or crash), the tab shows a **Restart** chip. One click rehydrates the same agent with the same working directory. Codex's restart chip also preserves the current account (see [Hot-swap Codex accounts](https://www.onorca.dev/docs/agents/codex-hot-swap)
).
...
3.  **Idle** — Orca detects the working→idle transition and fires an [agent-finished notification](https://www.onorca.dev/docs/notifications)
    .
