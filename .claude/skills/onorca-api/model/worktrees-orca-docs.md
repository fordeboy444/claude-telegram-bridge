...
- **Summary:** How Orca turns every feature or bug into its own git worktree.
...
*   Deleting a worktree removes both the directory and the branch (with confirmation). If git keeps a local branch because it may still have unmerged commits, Orca can offer a review step — see [Preserved branches](https://www.onorca.dev/docs/model/worktrees#preserved-branches)
    .
...
*   Link a GitHub PR, Linear issue, GitLab MR, or **Jira** issue from the name field (paste a Jira URL or search in **Jira** mode). Linked issues appear on the worktree card. See [Jira items drawer](https://www.onorca.dev/docs/review/jira)
    .

See [Ways to run Orca](https://www.onorca.dev/docs/ways-to-run)
.
...
In the workspace name field you can type Slack-style shortcodes (`:rocket:`) and pick from a shortcode suggestion popover. The same picker works when you double-click to rename a worktree, edit it in **Edit Worktree Details**, or type a shortcode in [Worktree Jump Palette](https://www.onorca.dev/docs/model/quick-open)
 (`Cmd-J`). The display name keeps the emoji; when Orca derives a git branch name it rewrites known emoji to readable shortcodes (e.g. 🚀 → `rocket`). Jump Palette search matches emoji-named workspaces by that shortcode-derived fragment.
...
*   **Other-client** workspaces — **Hide other-client workspaces** appears when a shared [Remote Orca Server](https://www.onorca.dev/docs/remote-servers)
     has workspaces created from another paired client; turn it on to keep this device's list to workspaces you created here. Empty `Cmd-J` recents and numeric shortcuts follow the same filter; typing a query still finds hidden rows.
*   **Detached HEAD** workspaces — checkouts sitting on a commit rather than a branch

Active filter count shows on the filter control; **Clear** resets only the filters that are on. Text search and [Worktree Jump Palette](https://www.onorca.dev/docs/model/quick-open)
 (`Cmd-J`) still reach workspaces hidden only by these filters once you type a query — the jump palette also has its own host/project filters (**Tab**).
...
A **Search** button at the top of the sidebar opens [Worktree Jump Palette](https://www.onorca.dev/docs/model/quick-open)
 (`Cmd-J`) — same surface, click-driven for users who don't lean on the keyboard for navigation. The status bar shows agent activity inline; unread worktrees are bolded rather than badged.
