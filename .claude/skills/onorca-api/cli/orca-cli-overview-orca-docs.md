# Orca CLI overview — Orca Docs

- **URL:** https://www.onorca.dev/docs/cli/overview
- **Summary:** Use the Orca CLI to script Orca from a terminal, manage worktrees, control agent terminals, automate the built-in browser, and install agent skills.
...
It ships with the desktop app; register it under [Settings → General → Orca CLI](https://www.onorca.dev/docs/settings)
.
...
See [Skills registry & MCP](https://www.onorca.dev/docs/cli/skills)
 for every installable Orca skill, including `orca skills install` / `orca skills update`.

![Orca CLI — drive worktrees, terminals, and the built-in browser from any shell](https://www.onorca.dev/whats-new/posters/orca-cli-demo.jpg)
...
For selectors, setup flags, parent/child worktrees, and the broader command map, see [Orca CLI reference](https://www.onorca.dev/docs/cli/reference)
.
...
For tracked multi-agent work, use [Orchestration](https://www.onorca.dev/docs/cli/orchestration)
...
Use `orca automations` to create, inspect, run, and remove scheduled Orca tasks from a shell. Start with [Scheduled automations](https://www.onorca.dev/docs/cli/automations)
 when you want a recurring prompt to run against a repo or an existing worktree.

Artifacts
---------

Share HTML or Markdown as public view links through the signed-in Orca account (`orca artifacts share|update|list|delete`). Publishing is **opt-in** under Settings → Artifacts. Command details: [CLI reference → Artifacts](https://www.onorca.dev/docs/cli/reference#artifacts)
.
