# Orca CLI reference — Orca Docs

- **URL:** https://www.onorca.dev/docs/cli/reference
- **Summary:** Commands, selectors, and agent-friendly patterns for driving Orca from a shell.
...
Register the CLI under [Settings -> Experimental -> CLI](https://www.onorca.dev/docs/settings)
, then check that it can reach Orca:
...
The result includes this machine, its registered [SSH targets](https://www.onorca.dev/docs/ssh)
, and paired [Remote Orca Servers](https://www.onorca.dev/docs/remote-servers)
. Use `--host local` for this machine, `--host ssh:<target-id>` for an SSH target, and `--environment <server-name>` for a paired server. SSH labels and paired-server names also resolve when they are unique; use the IDs from `host list` when names collide. If you put a machine name on the wrong selector, Orca reports the matching machine and the flag to use instead of returning an empty result.
...
`orca serve` starts a runtime server in the foreground without opening the desktop window. Use it for [Remote Orca Servers](https://www.onorca.dev/docs/remote-servers)
 or headless environments, and stop it with `Ctrl-C`.
...
See [Computer use](https://www.onorca.dev/docs/cli/computer-use)
 for the full workflow and permission setup.
...
`install` / `update` shell out to the same `npx skills` commands Settings uses. They do not contact the Orca runtime. See [Orca skills](https://www.onorca.dev/docs/cli/skills#keep-skills-up-to-date)
.
...
*   Use worktree comments for progress checkpoints. See [Worktree checkpoints](https://www.onorca.dev/docs/cli/worktree-checkpoints)
    .
*   Use [Orchestration](https://www.onorca.dev/docs/cli/orchestration)
     for tracked multi-agent dispatches instead of ad hoc terminal prompts.
