...
- `**src/tmux/question_handler.js**`: Formats `AskUserQuestion` tool calls into Telegram markdown cards with numbered emoji inline keyboards (`qa:<questionIndex>:<optionIndex>`, plus `submit_q`). Renders every question in the payload, builds the delayed tmux key sequence that answers each question page and submits on the review screen.
