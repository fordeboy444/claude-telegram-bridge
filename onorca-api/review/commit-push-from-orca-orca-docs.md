# Commit & push from Orca — Orca Docs

- **URL:** https://www.onorca.dev/docs/review/commit-push
- **Summary:** Commit & push from Orca — Orca documentation.
...
After the branch is pushed, use the Source Control panel's hosted review action to create the pull request or merge request. Confirm the base branch, title, description, and draft state before submitting. Bitbucket Cloud can create pull requests from the same composer; it has no draft PRs, so Draft is hidden. For GitHub, when the selected base already has an open PR you can **Stack this PR above #N** — see [Stacked pull requests](https://www.onorca.dev/docs/review/github#stacked-pull-requests)
.
...
*   [Hosted reviews, issues & Actions](https://www.onorca.dev/docs/review/github)
     — connect the provider that owns the review.
*   [Annotate AI Diff](https://www.onorca.dev/docs/review/annotate-ai-diff)
     — leave line notes before committing.
