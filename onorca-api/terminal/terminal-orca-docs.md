...
- **Summary:** Terminal — Orca documentation.

Search docs⌘K

Orca's terminal is the same xterm.js-based terminal VS Code uses, with a few additions tuned for AI-agent workflows.

![Ghostty-style terminal — import your Ghostty theme, font, and cursor on first launch](https://www.onorca.dev/whats-new/posters/ghostty-style-terminal.jpg)
...
Terminals are just tabs — see [Tabs, panes & split layouts](https://www.onorca.dev/docs/model/tabs-panes-splits)
. Splitting a terminal pane gives you two shells side by side.
...
Toggle: [Settings → Terminal → Allow TUI Clipboard Writes (OSC 52)](https://www.onorca.dev/docs/settings)
.
...
Terminal color themes are configurable under [Settings → Terminal](https://www.onorca.dev/docs/settings)
. Orca ships a library of popular themes and lets you customize any of them.

Ghostty import
--------------

If you use Ghostty, Orca can import its theme, font, and cursor config on first launch. You can re-run the import later from [Settings → Terminal → Import from Ghostty](https://www.onorca.dev/docs/settings)
.

Warp theme import
-----------------

If you've collected themes in Warp, click **Import themes from Warp** in the terminal-theme picker under [Settings → Terminal](https://www.onorca.dev/docs/settings)
 to bring them in as Orca terminal themes. Orca scans Warp's theme directory for the current OS (`~/.warp/themes` on macOS, `$XDG_DATA_HOME/warp-terminal/themes` on Linux, `%APPDATA%\warp\Warp\data\themes` on Windows) and lets you pick which YAML themes to import. The button next to it, **Import from YAML**, takes the same picker and points it at any folder of Warp-format `.yaml`/`.yml` files — useful when you store themes outside the default location.
...
The default shell on Windows is configurable between PowerShell, Command Prompt, and WSL under [Settings → Terminal](https://www.onorca.dev/docs/settings)
. WSL is offered automatically when `wsl.exe --status` succeeds. The **+** dropdown on the tab bar also shows a submenu so you can open a one-off tab in any shell without changing your default.
...
*   `Cmd-Alt-T` (macOS) — new agent tab using your default agent. On Linux and Windows this chord ships unbound; assign one under [Settings → Shortcuts](https://www.onorca.dev/docs/settings)
     (search for "New agent tab"). Each supported agent also has its own per-agent "New agent tab" action — bind a chord to launch a specific CLI directly without going through the default.
...
![Native key bindings via the kitty keyboard protocol — Shift+Enter and friends reach the agent CLI exactly like in your standalone terminal.](https://www.onorca.dev/whats-new/posters/keyboard-native.jpg)
...
*   Click the floating button on the edge of the window, or move the trigger to the status bar under [Settings → Terminal → Floating terminal](https://www.onorca.dev/docs/settings)
    .
