# Remote Orca Servers — Orca Docs

- **URL:** https://www.onorca.dev/docs/remote-servers
- **Summary:** Keep Orca running on another computer and connect from your laptop.
...
The easiest setup is the Orca desktop app on both computers, connected through [Tailscale](https://tailscale.com/)
. You do not need to run `orca serve` for this path.
...
![On the server: choose its Tailscale address, generate an access link, and copy the redacted pairing URL. The grant can be revoked later.](https://www.onorca.dev/docs/remote-server-share-desktop.png)
...
![On the client: name the server, paste its access link, and add it. The pairing code is redacted in this example.](https://www.onorca.dev/docs/remote-server-add-client.png)
...
When several paired clients share that server, the sidebar filter can offer **Hide other-client workspaces** so this device only lists workspaces it created. See [Worktrees → Sidebar layout](https://www.onorca.dev/docs/model/worktrees#sidebar-layout)
.
...
Use [SSH worktrees](https://www.onorca.dev/docs/ssh)
 when Orca on your laptop should own the runtime and use another machine only to run selected worktrees and terminals.
...
*   Compare every run mode in [Ways to run Orca](https://www.onorca.dev/docs/ways-to-run)
    .
*   Use [SSH worktrees](https://www.onorca.dev/docs/ssh)
     when the laptop should own the Orca runtime.
*   See the [Orca CLI reference](https://www.onorca.dev/docs/cli/reference)
     for `orca serve` flags and automation commands.
