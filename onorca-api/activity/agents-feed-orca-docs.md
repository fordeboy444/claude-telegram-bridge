# Agents feed — Orca Docs

- **URL:** https://www.onorca.dev/docs/activity
- **Summary:** Threaded feed of agent completions, blocking states, response previews, and worktree creation events across every worktree.

Search docs⌘K

The **Agents** entry in the sidebar opens a threaded feed of agent events across every worktree — completions, blocking questions, unread state, and worktree creation. It complements the per-worktree [notifications](https://www.onorca.dev/docs/notifications)
 view by giving you one chronological surface to triage from.
...
The Agents feed doesn't replace the [header bell](https://www.onorca.dev/docs/notifications)
 or system notifications — those still fire on agent-finished pings. The feed is the catch-up surface when you've been away from Orca: instead of scrolling each worktree to find what changed, scan it.

Next steps
----------

*   [Notifications & Inbox](https://www.onorca.dev/docs/notifications)
     — system pings, the header bell, and per-category tuning.
*   [Agents & sessions](https://www.onorca.dev/docs/model/agents-sessions)
     — how Orca tracks the working/idle/blocked transitions this feed surfaces.
