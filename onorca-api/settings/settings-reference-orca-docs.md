# Settings reference — Orca Docs

- **URL:** https://www.onorca.dev/docs/settings
- **Summary:** Settings reference — Orca documentation.
...
*   **GitHub API Budget** — remaining REST (core), Search, and GraphQL quota from the local `gh` CLI. Useful when PR checks or Tasks stop refreshing; see [Troubleshooting GitHub errors](https://www.onorca.dev/docs/github-errors)
     if the numbers look fine but GitHub still rate-limits live calls.
...
*   Scope filters for reviewing and editing the command list. The same list syncs to the [mobile companion](https://www.onorca.dev/docs/mobile)
    .
*   With remote or multi-host setups, commands are grouped by the Orca host that owns them (**Saved on**). Host ownership is separate from Global/Project scope and from where the command runs — see [Terminal → Quick Commands](https://www.onorca.dev/docs/terminal#quick-commands)
    .
...
*   Devtools opt-in.
*   **Remote server workspaces** — choose **This device** to render new paired-runtime browser pages on this desktop, or **Server (streamed)** to render them on the server. Traffic always goes through the remote server, and the choice applies to new pages only. See [Per-worktree browser → Remote workspaces](https://www.onorca.dev/docs/browser/overview#remote-workspaces)
    .
...
*   **Link Routing** — open http(s) links from the terminal, markdown, and editor in Orca's browser or the system browser. Nested **Hold Shift…** inverts that default for one click (`⇧⌘-click` / `Shift+Ctrl+click`). See [Per-worktree browser](https://www.onorca.dev/docs/browser/overview#link-routing)
    .
*   **Show terminal link actions** — **on by default**. A plain click on a terminal link opens a compact action popover. Turn off to require `⌘`\-click / `Ctrl`\-click. See [Terminal → Link actions](https://www.onorca.dev/docs/terminal#link-actions)
    .
*   **Default Search Engine** — used when you type non-URL text in the browser address bar or the [new-tab omnibox](https://www.onorca.dev/docs/model/quick-open#new-tab-omnibox)
    .
...
*   Share from an open local HTML page or Markdown editor with **Share as artifact**, or via `orca artifacts …`. See [CLI reference → Artifacts](https://www.onorca.dev/docs/cli/reference#artifacts)
    .
...
*   GitHub OAuth.
*   Linear API token.
*   Jira — Cloud (email + API token) or self-hosted Server/Data Center (PAT or username/password). See [Jira items drawer](https://www.onorca.dev/docs/review/jira)
    .
*   Bitbucket Cloud — **Connect** with **Email & API token** (default) or an **Access token**. Orca verifies the credential before saving. `ORCA_BITBUCKET_*` environment variables take precedence and hide Connect / Disconnect. Saved credentials stay on this machine — on a [Remote Orca Server](https://www.onorca.dev/docs/remote-servers)
    , set the env vars on the server instead. See [Hosted reviews](https://www.onorca.dev/docs/review/github)
    .
...
*   Kerberos hosts: `GSSAPIAuthentication` from OpenSSH config drives system OpenSSH auth (see [SSH worktrees](https://www.onorca.dev/docs/ssh)
    ).
...
*   **Worktree Shared Paths** — gitignored paths to materialize from the primary checkout into each new worktree (APFS clone-copy on macOS when possible, otherwise symlink). Complements repo-checked-in `worktree.sharedDirectories` in `orca.yaml` and `.worktreeinclude` (see [Worktrees](https://www.onorca.dev/docs/model/worktrees)\
    ).\
\
Floating Workspace\
------------------\
...
------------\
\
*   [Activity Page](https://www.onorca.dev/docs/activity)\
     — Slack-style worktree feed for agent events.\
...
*   [Agent hibernation](https://www.onorca.dev/docs/agents/hibernation)\
     — pause idle background agents and auto-resume them on reopen.\
*   **Agent Dashboard** — kanban of Needs You / Working / Done agents (optional Idle), with search and project/workspace/PR filters; open in-window or as a pop-out. **Show idle agents** lives on the dashboard board settings control, not here. See [Agents & sessions](https://www.onorca.dev/docs/model/agents-sessions#agent-dashboard)\
    .\
*   **Chat UI** — optional chat surface on supported agent terminals. See [Chat UI](https://www.onorca.dev/docs/agents/native-chat)\
    .\
*   **Cloud VM** — show setup controls and workspace **Run on** targets for repo-owned, on-demand environments (cloud sandboxes, VMs, or local Docker). Setup guide and recipe install live under this experimental toggle. See [Ways to run Orca](https://www.onorca.dev/docs/ways-to-run#4-cloud-vms-per-workspace-environments)\
    .\
