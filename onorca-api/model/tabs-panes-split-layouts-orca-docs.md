# Tabs, panes & split layouts — Orca Docs

- **URL:** https://www.onorca.dev/docs/model/tabs-panes-splits
- **Summary:** Drag-to-split panes, tab groups, and pinned boundaries.
...
![Drag a tab to a pane edge to split — terminals, diffs, and browser tabs side by side](https://www.onorca.dev/whats-new/posters/tab-split.jpg)
...
\
Remap under [Settings → Shortcuts](https://www.onorca.dev/docs/settings)\
. Existing installs keep customized overrides in `~/.orca/keybindings.json`.\
\
Split panes\
-----------\
\
...
\
![Any tab type can split with any other — agent terminal, diff, browser, editor, PR view all coexist in one pane tree.](https://www.onorca.dev/whats-new/split-anything.webp)\
\
